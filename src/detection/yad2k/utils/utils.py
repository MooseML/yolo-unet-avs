...
def preprocess_image(img, model_image_size):
    """
    Preprocess image for YOLO model. Accepts either a file path or PIL.Image
    """
...
#         font='../data/detection/fonts/FiraMono-Medium.otf',
...
#         top    = min(int(round(box[0])), image.height - 1)
#         left   = min(int(round(box[1])), image.width - 1)
#         bottom = max(int(round(box[2])), top + 1)  # ensure bottom > top
#         right  = max(int(round(box[3])), left + 1)  # ensure right > left
...
def draw_boxes(image, boxes, box_classes, class_names, scores=None):
    """
    Draw bounding boxes with class names and optional scores on the image
    """
    font = ImageFont.truetype(
        font='../data/detection/fonts/FiraMono-Medium.otf',
        size=int(0.03 * image.size[1] + 0.5)
    )
    thickness = (image.size[0] + image.size[1]) // 300
    colors = get_colors_for_classes(len(class_names))
    ...
    for i, c in enumerate(box_classes):
        ...
        label = f"{box_class} {score:.2f}" if scores is not None else box_class

        draw = ImageDraw.Draw(image)
        label_size = draw.textsize(label, font)

        # Extract and clamp coordinates
        y_min = int(round(float(box[0])))
        x_min = int(round(float(box[1])))
        y_max = int(round(float(box[2])))
        x_max = int(round(float(box[3])))

        # ensure coordinates are in correct order and w/in image bounds
        top = max(0, min(y_min, y_max, image.height - 1))
        left = max(0, min(x_min, x_max, image.width - 1))
        bottom = max(top + 1, min(max(y_min, y_max), image.height))
        right = max(left + 1, min(max(x_min, x_max), image.width))
        ...
        text_box = [tuple(text_origin),
                    (text_origin[0] + label_size[0], text_origin[1] + label_size[1])]
        draw.rectangle(text_box, fill=colors[c])
        draw.text(text_origin, label, fill=(0, 0, 0), font=font)
